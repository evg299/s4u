...
<html>
    <head>
        <title><?= $v_params['sys_name'] ?> | <?= $v_params['img_name']; ?> | Альбомы</title>
                    ...
                    <div class="sc_block">
                        ...
                        <div style="padding-top: 10px; margin: 0 13px;">
                            <div style="margin-bottom: 15px; padding-bottom: 5px; border-bottom: dotted 1px gainsboro;">
                                <div><a href="<?= $v_params['url_prefix'] . "album?act=show"; ?>"><h3>0. Картинки без альбома</h3></a></div>
                            ...
                            <? if (count($v_params['img_albums'])): ?>
                                <? foreach ($v_params['img_albums'] as $imgAlbum): ?>
                                    <div style="margin-bottom: 15px; padding-bottom: 5px; border-bottom: dotted 1px gainsboro;">
                                        <div><a href="<?= "album?act=show&alb_id=" . $imgAlbum['id']; ?>"><h3><?= $count++ . ". " . $imgAlbum['name']; ?></h3></a></div>
                                        <div><?= $imgAlbum['description']; ?></div>
                                        <div style="text-align: right;"><a href="<?= "album?act=upd&alb_id=" . $imgAlbum['id']; ?>">Переименовать</a></div>
                                        <div style="text-align: right;"><a href="<?= "album?act=del&alb_id=" . $imgAlbum['id']; ?>">Удалить</a></div>
                                    </div>
                                <? endforeach; ?>
                            <? endif; ?>
                            <div class="button" style="width: 130px; margin: 3px auto;"><a class="a_clear" href="<?= "album?act=add"; ?>">Создать альбом</a></div>
                        </div>
                    </div>
                </td>
            </tr>
